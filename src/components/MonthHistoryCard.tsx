      ...
      >
        <div className="w-full flex items-center justify-between">
          ...
          <div className="flex flex-col items-center">
            ...
            <p
              className={`text-4xl font-bold mt-1 transition-all ${
                monthHours > 0
                  ? theme.text
            ...
            {totalLdcHours > 0 && (
              <p
                className={`text-sm font-semibold mt-2 ${theme.text} ${privacyBlur}`}
              >
          ...
          {hasEntries ? (
            <ChevronDownIcon
              className={`w-6 h-6 text-slate-400 dark:text-slate-500 transition-transform duration-300 ${
        ...
        <div className="overflow-hidden">
          <div className="p-4 border-t border-slate-200/80 dark:border-slate-700">
            <p className="text-sm font-semibold text-slate-600 dark:text-slate-300 mb-3 text-center">
              {dailyEntries.length}{" "}
              {dailyEntries.length === 1 ? "informe" : "informes"} este mes
            </p>
            <ul className="space-y-2 max-h-48 overflow-y-auto">
              {dailyEntries.map((entry, index) => (
                ...
                >
                  ...
                  <div
                    className={`text-sm font-bold transition-all flex items-center gap-2 ${privacyBlur}`}
                  >
                    {entry.ldcHours && entry.ldcHours > 0 && (
                      <span className={`${theme.text}`}>
                        {isPrivacyMode ? "0:00" : hoursToHHMM(entry.ldcHours)}{" "}
                        LDC
                      </span>
                    )}
                    {entry.hours > 0 && (
