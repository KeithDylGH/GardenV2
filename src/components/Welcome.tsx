...
const StreakCounterAnimation: React.FC<{ isActive: boolean; theme: Theme }> = ({
  isActive,
  theme,
}) => {
  ...
  return (
    <div
      className={`text-7xl font-bold ${theme.text} tracking-tighter w-28 text-left animate-fadeIn`}
    >
      {count}
    </div>
  );
...
const GhostModeVisualExample: React.FC<{ isActive: boolean }> = ({
  isActive,
}) => {
...
const TimerVisualExample: React.FC<{ theme: Theme }> = ({ theme }) => (
  <div className="flex flex-col items-center space-y-4 h-28 justify-center">
    ...
    <div
      className={`w-14 h-14 rounded-full flex items-center justify-center text-white shadow-lg ${theme.bg}`}
    >
...
type Slide = {
  ...
  description?:
    | React.ReactNode
    | ((name: string, role: UserRole) => React.ReactNode);
  step?: number;
};
...
const getServiceYearMonthsForSetup = () => {
  ...
  for (let i = 0; i < 12; i++) {
    const monthDate = new Date(
      serviceYearStartDate.getFullYear(),
      serviceYearStartDate.getMonth() + i,
      1
    );
    if (
...
const slideData: Slide[] = [
  ...
  },
  {
    ...
    description:
      "Cada día que registres horas, tu racha aumentará. Los fines de semana y tu día de descanso semanal protegen tu racha para que no la pierdas.",
  },
  {
    type: "feature",
    icon: (theme: Theme, isActive?: boolean) => (
  ...
  },
  {
    ...
    description:
      "Inicia el temporizador cuando comiences a predicar y la app registrará el tiempo por ti. Simple y preciso.",
  },
  {
    type: "feature",
    ...
    description:
      "Navega por tu servicio en un calendario interactivo. Toca cualquier día para añadir o editar tus horas fácilmente.",
  },
  {
    type: "feature",
    ...
    description:
      "Con el Modo Privacidad, puedes ocultar tus horas con un solo toque, ideal para cuando estás en público.",
  },
  {
    type: "feature",
    icon: () => (
    ...
    description:
      "Disfruta de una interfaz limpia y personaliza la apariencia de la app con diferentes formas, colores y temas.",
  },
  {
    type: "setup",
    step: 1,
...
const RoleButton: React.FC<{
  ...
  theme: Theme;
}> = ({ label, value, current, onClick, theme }) => (
  <button
    type="button"
...
const ChangingWordComponent: React.FC<{ isActive: boolean; theme: Theme }> = ({
  isActive,
  theme,
}) => {
  const changingWords = useMemo(
    () => ["organizar", "mejorar", "disfrutar", "planificar", "fortalecer"],
    []
  );
  ...
  return (
    <div className="relative h-14 md:h-16 w-full">
      ...
      {changingWords.map((word, index) => (
        <h1
          key={word}
          className={`absolute w-full text-4xl md:text-5xl font-bold bg-clip-text text-transparent bg-gradient-to-r ${
            theme.gradientFrom
          } ${
            theme.gradientTo
          ...
          style={{
            transitionDelay: `${index === wordIndex ? "200ms" : "0ms"}`,
          }}
        >
          {word}
        </h1>
...
const Welcome: React.FC<WelcomeProps> = ({
  ...
  const [previousHoursInput, setPreviousHoursInput] = useState<{
    [key: string]: string;
  }>({});
  ...
  const getSlideTheme = (slideIndex: number, slideTitle: string): Theme => {
    if (
      slideIndex < customizeSlideIndex &&
      slideTitle !== "Dale tu toque personal"
    ) {
  ...
  const handleNext = () => {
    if (currentSlide < slides.length - 1) {
      ...
      onFinish({
        ...
        protectedDay,
        protectedDaySetDate:
          protectedDay !== null ? new Date().toISOString() : null,
      });
    }
  };
  ...
  const isNextDisabled = useMemo(() => {
    ...
    switch (slideTitle) {
      ...
      case "Ponte al día": {
        ...
        if (
          currentMonthHours.trim() === "" ||
          isNaN(currentHoursNum) ||
          currentHoursNum < 0
        ) {
          return true;
        }
  ...
  return (
    ...
    >
      ...
      >
        {(currentSlide < customizeSlideIndex ||
          isLastSlide ||
          currentSlideData.type === "title" ||
          currentSlideData.type === "feature") && (
          <div className="absolute -bottom-1/2 -left-1/3 w-[200%] max-w-5xl aspect-square bg-cyan-500/20 rounded-full blur-3xl opacity-90" />
        )}
      </div>
        ...
        {currentSlideData.step && currentSlide !== 0 && (
          <span
            className={`text-sm font-bold ${
              isEffectiveLight ? "text-slate-500" : "text-slate-400"
            } animate-fadeIn`}
          >
            {currentSlideData.step} / {TOTAL_SETUP_STEPS}
          </span>
        )}
      </div>
          ...
          >
            {slides.map((slide, index) => {
              ...
              return (
                <div
                  key={index}
                  className={`w-full h-full flex-shrink-0 flex flex-col relative ${
                    slide.type === "title"
                ...
                >
                    ...
                    (slide.title === "FinalSlide" ? (
                      ...
                      <>
                        <div className="mb-6 h-28 flex items-center justify-center">
                          {slide.icon &&
                            slide.icon(slideTheme, currentSlide === index)}
                        </div>
                        <h2
                          className={`text-3xl font-bold ${mainTextColor} mb-3 tracking-tight`}
                        >
                          {slide.title}
                        </h2>
                        <p
                          className={`${subTextColor} max-w-sm leading-relaxed`}
                        >
                          {(() => {
                  ...
                  {(slide.type === "customize" || slide.type === "setup") && (
                    <>
                      <h2
                        className={`text-3xl font-bold ${mainTextColor} mb-4 tracking-tight`}
                      >
                        {slide.title}
                      </h2>
                      <div
                        className={`w-full max-w-sm text-left space-y-4 ${cardBgColor} p-4 rounded-xl shadow-inner h-full max-h-96 flex flex-col`}
                      >
                        ...
                        {slide.title === "Define tu meta mensual" && (
                          <div>
                            <label
                              ...
                              className={`text-sm font-medium ${subTextColor}`}
                            >
                              Basado en tu rol, te sugerimos una meta, pero
                              puedes ajustarla.
                            </label>
                            <input
                              id="goal-input"
                        ...
                        {slide.title === "Ponte al día" && (
                          <div className="flex-grow flex flex-col min-h-0">
                            <div className="flex-shrink-0 mb-4">
                              ...
                              >
                                {
                                  "¿Cuántas horas has hecho hasta hoy en el mes actual?"
                                }
                              </label>
                              <input
                                type="text"
                              ...
                              <div className="mt-2 space-y-2 overflow-y-auto pr-2 flex-grow">
                                {monthsForSetup.map((date) => {
                                  ...
                                  return (
                                    ...
                                    >
                                      <span
                                        className={`${subTextColor} capitalize text-sm font-medium`}
                                      >
                                        {monthName}
                                      </span>
                                      <input
                                        ...
                                        placeholder="0"
                                        value={
                                          previousHoursInput[dateKey] || ""
                                        }
                                        onChange={(e) =>
                                          setPreviousHoursInput((prev) => ({
                                            ...prev,
                        ...
                        {slide.title === "Tu Rutina Semanal" && (
                          <div className="space-y-4">
                              ...
                              <div className="flex justify-center gap-1.5 mt-2">
                                {weekDaysForMeeting.map((day) => (
                                  <button
                                    ...
                                    onClick={() =>
                                      handleMeetingDayToggle(day.value)
                                    }
                            ...
                            <div>
                              ...
                              <p className="text-xs text-slate-400 mt-1">
                                Este día protegerá tu racha, además del fin de
                                semana. Podrás cambiarlo cada 7 días.
                              </p>
                              <div className="flex justify-center gap-2 mt-2">
                                {weekDaysForRest.map((day) => {
                                  const isMeetingDay = meetingDays.has(
                                    day.value
                                  );
                                  return (
                                    <button
                                      ...
                                      onClick={() =>
                                        setProtectedDay((p) =>
                                          p === day.value ? null : day.value
                                        )
                                      }
                                      disabled={isMeetingDay}
                                      title={
                                        isMeetingDay
                                          ? "Este día ya es un día de reunión"
                                          : ""
                                      }
                        ...
                        {slide.title === "Dale tu toque personal" && (
                          <div className="space-y-3">
                            ...
                            <div>
                              <label
                                className={`block text-center text-sm font-semibold ${subTextColor} mb-2`}
                              >
                                Forma
                              </label>
                              ...
                              <div className="grid grid-cols-8 gap-2">
                                {THEME_LIST.map((themeOption) => (
                                  <button
                                    ...
                                    onClick={() =>
                                      setThemeColor(themeOption.name)
                                    }
          ...
          <>
              ...
              {slides.map((_, index) => (
                <div
                  ...
                  className={`w-2.5 h-2.5 rounded-full transition-all duration-300 ${
                    currentSlide === index
                      ? `${isEffectiveLight ? "bg-slate-800" : "bg-white"} w-6`
                      : `${isEffectiveLight ? "bg-slate-500" : "bg-slate-600"}`
                  }`}
                />
              ))}
            </div>
            ...
            <button
              ...
              className={`w-full max-w-sm mx-auto py-3 rounded-lg font-bold text-lg transition-all disabled:opacity-50 disabled:cursor-not-allowed ${
                ...
                currentSlideData.type === "title"
                  ? `${
                      isEffectiveLight ? "text-slate-500" : "text-white"
