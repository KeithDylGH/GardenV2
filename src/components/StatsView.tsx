...
const StatsView: React.FC<StatsViewProps> = ({
  ...
  const stats = useMemo(() => {
    ...
    const yearMonths = [...Array(12).keys()].map((i) => {
      ...
      const year =
        parseInt(currentServiceYear.split("-")[0]) + Math.floor((8 + i) / 12);
      return { year, month: monthIndex };
    });

    yearMonths.forEach(({ year, month }) => {
      const summaryKey = `${year}-${String(month + 1).padStart(
        2,
        "0"
      )}-SUMMARY`;
    ...
    // 2. Annual Projection
    const monthsPassed =
      (currentMonth - 8 < 0 ? currentMonth + 4 : currentMonth - 8) +
      currentDayOfMonth / 31;
    const averageMonthlyHours =
      monthsPassed > 0 ? serviceYearTotal / monthsPassed : 0;
    const annualProjection = averageMonthlyHours * 12;

    return {
      serviceYearTotal: isPrivacyMode
        ? privacyValue
        : hoursToHHMM(serviceYearTotal),
      annualProjection: isPrivacyMode
        ? privacyValue
        : Math.round(annualProjection),
    };
