...
export const ALL_ACHIEVEMENTS: Achievement[] = [
  ...
  },
  {
    ...
    tiers: [1, 3, 5, 10],
    check: (state: AppState) => {
      const studyCount = state.activities.filter(
        (act) => act.type === "study"
      ).length;
  ...
  },
  {
    ...
    check: (state: AppState) => {
      return {
        unlocked: state.currentHours >= 60,
        currentProgress: state.currentHours,
      };
    },
  },
  {
  ...
  },
  {
    ...
    check: (state: AppState) => {
      const progress =
        state.goal > 0 ? (state.currentHours / state.goal) * 100 : 0;
  ...
  },
  {
    ...
    description: () =>
      `Registra horas todos los días no protegidos de una semana.`,
    ...
    check: (state: AppState) => {
      ...
      for (let i = 0; i < 7; i++) {
        ...
        const isProtected =
          dayOfWeek === 0 ||
          dayOfWeek === 6 ||
          (state.protectedDay !== null && dayOfWeek === state.protectedDay);
